# MaterialGPT — V1-V6 lite + Harness/Evaluation (Parte 5) + Quantum Chemistry (QRA)
...
## Quantum Chemistry — de `Quantum_Research_Assistant.md` (`backend/app/quantum/`)
...
* **Quantum Chemistry (QRA)**: VQE de H₂ validado em produção via `/api/chat` real
  ...
  água testada também: recusa rápida e honesta, sem travar nem fabricar energia.
...
* Expandir o `quantum_chemistry` pra LiH e HeH⁺ (mesma classe de 4 qubits do H2, então
  ...
  Adicionar golden dataset/teste de regressão pra essa intenção também.
